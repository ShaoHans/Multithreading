...
class Program
{
    ...
    static async Task ProcessIOAsync()
    {
        using (
        ...
        {
            Console.WriteLine(
                $"1.使用I/O线程，线程Id：{Environment.CurrentManagedThreadId}，是否异步：{stream.IsAsync}"
            );
        ...
        {
            Console.WriteLine(
                $"2.使用I/O线程，线程Id：{Environment.CurrentManagedThreadId}，是否异步：{stream.IsAsync}"
            );
            byte[] buffer = Encoding.UTF8.GetBytes(CreateFileContent());
            var writeTask = Task.Factory.FromAsync(
                stream.BeginRead,
        ...
        {
            Console.WriteLine(
                $"3.使用I/O线程，线程Id：{Environment.CurrentManagedThreadId}，是否异步：{stream.IsAsync}"
            );
        ...
        {
            Console.WriteLine(
                $"4.使用I/O线程，线程Id：{Environment.CurrentManagedThreadId}，是否异步：{((FileStream)sw.BaseStream).IsAsync}"
            );
            await sw.WriteAsync(CreateFileContent());
        }
